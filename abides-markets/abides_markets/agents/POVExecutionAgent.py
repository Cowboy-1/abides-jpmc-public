...
class POVExecutionAgent(TradingAgent):
    ...
    def receiveMessage(self, currentTime, msg):
        ...
        if self.rem_quantity > 0 and \
           ...
           and currentTime > self.start_time:
            ...
            logger.info(f'[---- {self.name} - {currentTime} ----]: TOTAL TRANSACTED VOLUME IN THE LAST {self.look_back_period} = {self.transacted_volume[self.symbol]}')  # CHANGED
            logger.info(f'[---- {self.name} - {currentTime} ----]: MARKET ORDER PLACED - {qty}')  # CHANGED
    ...
    def handleOrderAcceptance(self, currentTime, msg):
        ...
        logger.info(f'[---- {self.name} - {currentTime} ----]: ACCEPTED QUANTITY : {accepted_qty}')  # CHANGED
    ...
    def handleOrderExecution(self, currentTime, msg):
        ...
        logger.info(f'[---- {self.name} - {currentTime} ----]: LIMIT ORDER EXECUTED - {executed_order.quantity} @ {executed_order.fill_price}')  # CHANGED
        logger.info(f'[---- {self.name} - {currentTime} ----]: EXECUTED QUANTITY: {executed_qty}')  # CHANGED
        logger.info(f'[---- {self.name} - {currentTime} ----]: REMAINING QUANTITY (NOT EXECUTED): {self.rem_quantity}')  # CHANGED
        logger.info(f'[---- {self.name} - {currentTime} ----]: % EXECUTED: {round((1 - self.rem_quantity / self.quantity) * 100, 2)} \n')  # CHANGED
